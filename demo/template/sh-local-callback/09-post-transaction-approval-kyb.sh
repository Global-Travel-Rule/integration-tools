...
encryptedPayload=$(curve25519_from_file $myPriv $LOCAL_CALLBACK_SERVER_PUBLIC_KEY $basedir/../ivms/post_ivms_kyb.json)
...
resp=$(curl --no-progress-meter --location --request POST "http://localhost:8080/callback" \
    ...
    \"callbackData\": {
        ...
        \"address\": \"$TESTER_KYB_ADDRESS\",
        \"tag\": \"$TESTER_KYB_ADDRESS_TAG\",
        \"txId\": \"$TESTER_KYB_TX_ID\",
        \"network\": \"$TESTER_KYB_NETWORK_SYMBOL\",
